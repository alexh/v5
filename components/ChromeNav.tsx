...
// ---- Outer component: DOM wrapper, measurement, canvas mount ----
...
const ChromeNav: React.FC<ChromeNavProps> = ({ className = '', children }) => {
  ...
  const active = fontsReady && positions.length > 0 && wrapperSize.w > 0

  return (
    <div
      ref={wrapperRef}
      className={`${className} ${active ? 'chrome-nav-active' : ''}`}
